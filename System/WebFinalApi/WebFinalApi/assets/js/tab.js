...
!function(window) {
    ...
    Tab.prototype.active = function($element, $container, callback) {
        ...
        if ($element.hasClass("tab-nav-item"))
        {
            //target
            //alert($element.children(":first").get(0).innerText);

            //alert();
            //data init
            var data = GetUserOrderList($($element.children(":first").get(0)).data("target"));

            OrderListInit(data, $($element.children(":first").get(0)).data("target"));
        }
    };
